...
function parseArgs() {
  ...
  return {
    mode: get("--mode", "extremes") as BollingerScanMode,
...
async function main() {
  ...
  const matched = constituentResults
    .filter((s) => matchesBollingerMode(s, opts.mode, opts.squeezeThresholdPct))
    .sort((a, b) => bollingerRankKey(a, opts.mode) - bollingerRankKey(b, opts.mode))
    .slice(0, opts.limit);
