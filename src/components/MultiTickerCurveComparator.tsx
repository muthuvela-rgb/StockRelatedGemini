...
const COMPARATOR_COLUMNS: ColumnDefinition<MultiTableSortKey>[] = [
  ...
  { key: "rsi_14", label: "RSI (14)", defaultDirection: "asc", numeric: true, extractor: (item: any) => item.rsi_14 ?? -1 },
  { key: "knee_point", label: "Sweet-Spot Knee Yield", defaultDirection: "desc", numeric: true, extractor: (item: any) => item.knee_point?.annualized_return_cash_secured ?? -1 },
];

const COMPARATOR_PRESETS: SortPreset<MultiTableSortKey>[] = [
  {
    label: "Yield Leaders: Cash Return ➔ IV ➔ Ticker",
    description: "Highest annualized cash-secured yield across compared tickers",
    criteria: [
      { field: "avg_cash_return", direction: "desc" },
      { field: "avg_iv", direction: "desc" },
      { field: "ticker", direction: "asc" },
    ],
  },
  {
    label: "Oversold High-Yield: RSI ➔ Cash Return ➔ IV",
    description: "Lowest RSI (deepest oversold) with highest cash-secured return",
    criteria: [
  ...
  {
    label: "Alphabetical: Ticker ➔ Cash Return",
    description: "Ordered alphabetically by ticker symbol",
    criteria: [
      { field: "ticker", direction: "asc" },
      { field: "avg_cash_return", direction: "desc" },
    ],
...
export const MultiTickerCurveComparator: React.FC<MultiTickerCurveComparatorProps> = ({
  ...
  const [tableSortCriteria, setTableSortCriteria] = useState<SortCriterion<MultiTableSortKey>[]>([
    { id: "1", field: "avg_cash_return", direction: "desc" },
  ]);

  const handleTableSort = (field: MultiTableSortKey, isShift: boolean = false) => {
    const defaultDir = field === "ticker" || field === "target_strike" || field === "rsi_14" ? "asc" : "desc";
    setTableSortCriteria((prev) => handleHeaderClick(field, isShift, prev, defaultDir));
  };

  const sortedTickerSummaries = useMemo(() => {
    return applyHierarchicalSort(tickerSummaries, tableSortCriteria, COMPARATOR_COLUMNS);
  }, [tickerSummaries, tableSortCriteria]);
      ...
      {data && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
          ...
          <HierarchicalSortControl
            ...
            availableColumns={COMPARATOR_COLUMNS}
              ...
              <thead className="bg-slate-950/80 text-slate-400 uppercase tracking-wider font-semibold border-b border-slate-800">
                <tr>
                  ...
                  <TableSortHeader
                    ...
                    criteria={tableSortCriteria}
                    onSortClick={handleTableSort}
                    className="py-3 px-3 text-emerald-400"
                  />
                  <TableSortHeader
                    ...
                    criteria={tableSortCriteria}
                    onSortClick={handleTableSort}
                    className="py-3 px-3 text-blue-400"
                  />
                  <TableSortHeader
                    field="avg_iv"
                    label="Avg IV"
